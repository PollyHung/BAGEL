...
#' @export
run_biscut_pipeline <- function(cancer_folder, results_dir, cores = 4, cleanup = TRUE) {
...
#' Run BISCUT Pipeline on Multiple Cancer Types
#'
#' Executes BISCUT analysis on multiple cancer types in parallel or sequentially.
#' This function provides a convenient way to process multiple cancer datasets
#' using the same BISCUT pipeline.
#'
#' @param cancer_folders Character vector of cancer folder names
#' @param results_dir Character, path to the directory containing cancer-specific results
#' @param cores Integer, number of CPU cores to use per cancer type (default: 4)
#' @param parallel Logical, whether to run cancer types in parallel (default: FALSE)
#' @param max_parallel Integer, maximum number of cancer types to run in parallel (default: 2)
#'
#' @return Named list where each element corresponds to a cancer type and contains
#'   the result from run_biscut_pipeline()
#'
#' @examples
#' \dontrun{
#' # Run BISCUT on multiple cancer types
#' cancer_types <- c("breast_invasive_carcinoma", "lung_adenocarcinoma", "colon_adenocarcinoma")
#' 
#' results <- run_biscut_batch(
#'   cancer_folders = cancer_types,
#'   results_dir = "/path/to/cancer_results",
#'   cores = 4,
#'   parallel = TRUE,
#'   max_parallel = 2
...
#'
#' # Summarize results
#' successful <- sapply(results, function(x) x$status == "success")
#' cat("Successful analyses:", sum(successful), "/", length(results), "\n")
#' }
#'
#' @export
run_biscut_batch <- function(cancer_folders, results_dir, cores = 4, 
                            parallel = FALSE, max_parallel = 2) {
  
  if (length(cancer_folders) == 0) {
    stop("No cancer folders provided")
  }
  
  cat("=== BISCUT Batch Analysis ===\n")
  cat("Processing", length(cancer_folders), "cancer types\n")
  cat("Results directory:", results_dir, "\n")
  cat("Cores per analysis:", cores, "\n")
  ...
  if (parallel && requireNamespace("parallel", quietly = TRUE)) {
    # Run in parallel
    n_cores <- min(max_parallel, length(cancer_folders))
    cat("Running", n_cores, "cancer types in parallel\n")
    
    cl <- parallel::makeCluster(n_cores)
    tryCatch({
      # Export necessary functions and variables to cluster
      parallel::clusterEvalQ(cl, library(BAGEL))
      parallel::clusterExport(cl, c("results_dir", "cores"), envir = environment())
      
      results <- parallel::parLapply(cl, cancer_folders, function(cancer_folder) {
        run_biscut_pipeline(cancer_folder, results_dir, cores)
      })
      
      names(results) <- cancer_folders
    ...
    # Run sequentially
    cat("Running cancer types sequentially\n")
    
    for (cancer_folder in cancer_folders) {
      cat("\n--- Processing:", cancer_folder, "---\n")
      results[[cancer_folder]] <- run_biscut_pipeline(cancer_folder, results_dir, cores)
    }
  }
  ...
  successful <- sapply(results, function(x) x$status == "success")
  errors <- sapply(results, function(x) x$status == "error")
  no_data <- sapply(results, function(x) x$status == "no_seg_file")
  
  cat("Successful:", sum(successful), "/", length(results), "\n")
  cat("Errors:", sum(errors), "\n")
  cat("No data:", sum(no_data), "\n")
  
  if (sum(errors) > 0) {
    cat("\nFailed cancer types:\n")
    failed_types <- names(results)[errors]
    for (failed in failed_types) {
      cat(" -", failed, ":", results[[failed]]$error, "\n")
    }
  }
